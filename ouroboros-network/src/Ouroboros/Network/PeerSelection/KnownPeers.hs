...
data KnownPeerInfo = KnownPeerInfo {
       ...
       knownPeerSharing   :: !(Maybe PeerSharing),
...
insert :: Ord peeraddr
       => Map peeraddr (Maybe PeerSharing, PeerAdvertise, LedgerPeer)
       -> KnownPeers peeraddr
       -> KnownPeers peeraddr
insert peeraddrs
  ...
  case Map.lookup peeraddr allPeers of
    ...
    Just (KnownPeerInfo i b _ pa lp) ->
      case mbps of
        ...
        Just ps ->
          let allPeers' =
                Map.insert peeraddr (KnownPeerInfo i b (Just ps) pa lp) allPeers
