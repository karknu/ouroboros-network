...
inboundPeers PeerSelectionActions{
  ...
  Guarded Nothing $ do
    ...
    return $ \_ ->
      let peerAdvertiseValue a =
            if KnownPeers.member a knownPeers
               then Nothing
               else Just DoAdvertisePeer
          -- If peer happens to already be present in the Known Peer set
          -- 'insert' is going to do its due diligence before adding.
          newEntry    = Map.singleton addr (Just ps, peerAdvertiseValue addr, Nothing)
