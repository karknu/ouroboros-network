...
import           Control.Concurrent.Class.MonadSTM.Strict (MonadSTM, StrictTVar,
                     atomically, modifyTVar, newTVarIO, retry)
...
newtype PeerSharingController peer m = PeerSharingController {
  ...
  requestQueue :: MVar m (PeerSharingAmount, MVar m [peer])
}
...
bracketPeerSharingClient (PeerSharingRegistry registry) peer k = do
  ...
  newPSController <- PeerSharingController <$> newEmptyMVar
...
peerSharingClient controlMessageSTM
  ...
  mbTerminated <- atomically
                $ runFirstToFinish
                $ FirstToFinish (return (Just (takeMVar requestQueue)))
               <> FirstToFinish (do
