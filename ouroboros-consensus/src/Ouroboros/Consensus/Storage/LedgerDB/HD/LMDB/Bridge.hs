...
{-| Alternatives to LMDB operations that do not rely on Serialise instances
  ...
  TODO: It could be possible to upstream changes to @lmdb-simple@ that make the
  API compatible with both explicit (CBOR encodings/decoders) and implicit
  (@'Serialise'@ constraints) serialisation. This would constitute a more
  profound change in the @lmdb-simple@ API, however. Furthermore, the @'get@',
  @'put'@ and @'delete'@ functions (and auxiliary definitions) could be made to
  operate on bytestrings directly, moving the responsibility of
  serialisation/deserialisation up a few levels.
-}
module Ouroboros.Consensus.Storage.LedgerDB.HD.LMDB.Bridge (
