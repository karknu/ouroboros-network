    ...
    modules = [
      ...
      ({ pkgs, ... }:
        lib.mkIf pkgs.stdenv.hostPlatform.isWindows {
          ...
          packages.ouroboros-network-protocols-test.flags.cddl = false;
      ...
      ({ pkgs, ... }:
        lib.mkIf (!pkgs.stdenv.hostPlatform.isWindows) {
          packages.ouroboros-network-protocols-test.flags.cddl = true;
          packages.ouroboros-network-protocols-test.components.tests.cddl.build-tools =
            [ pkgs.cddl pkgs.cbor-diag ];
          packages.ouroboros-network-protocols-test.components.tests.cddl.preCheck =
            "export HOME=`pwd`";
        })
